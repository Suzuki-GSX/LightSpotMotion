...
void MainWindow::serialPortReadData()//数据接收
{
    ...
    const char *c = data;
    if(data[0]=='1'&&data.size()>8)
    {
        whiteBoard->SpotCoorX = (c[2]*100)+(c[3]*10)+c[4];
        whiteBoard->SpotCoorY = (c[6]*100)+(c[7]*10)+c[8];
        ui->CoorXLineEdit->setText(QString("%1").arg(whiteBoard->SpotCoorX));
        ui->CoorYLineEdit->setText(QString("%1").arg(whiteBoard->SpotCoorY));
    }
